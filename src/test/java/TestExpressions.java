...
public class TestExpressions {
    ...
    public void testSimplification() {
        Expr e = new MultOp(new MultOp(new SymbExpr(M.Ident("a")), new Const(0.5f)), new Const(0.5f));
        assertEquals("0.25F * a", e.getSimplifiedAST().toString());
    }

    @Test
    public void testSimplification1() {
        Expr e = new AddOp(new AddOp(new SymbExpr(M.Ident("a")), new Const(0.5f)), new Const(0.5f));
        assertEquals("1.0F + a", e.getSimplifiedAST().toString());
    }

    @Test
    public void testSimplification2() {
        Expr e = new MultOp(new SymbExpr(M.Ident("a")), new Const(0.f));
        assertEquals("0.0F", e.getSimplifiedAST().toString());
    }

    @Test
    public void testSimplification3() {
        Expr e = new MultOp(new SymbExpr(M.Ident("a")), new Const(1.0f));
    ...
    public void testSimplification4() {
        Expr e = new AddOp(new SymbExpr(M.Ident("a")), new Const(0.0f));
        assertEquals("a", e.getSimplifiedAST().toString());
    }
}
